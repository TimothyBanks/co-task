...
std::atomic<uint32_t>& operation_context::execution_count() {
  if (!impl) {
    throw cotask_exception{"impl is invalid"};
  }
...
const std::atomic<uint32_t>& operation_context::execution_count() const {
  if (!impl) {
    throw cotask_exception{"impl is invalid"};
  }
  return impl->execution_count;
...
size_t operation_context::address() const {
